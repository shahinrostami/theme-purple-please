...
const callEventHandlerListeners = (eventHandler, arg0, arg1) =>
  func__namespace.callAll(eventHandler.l, [arg0, arg1]);
...
var Y = /*#__PURE__*/Object.freeze({
  ...
  LazyStructReader: LazyStructReader,
  logUpdate: logUpdate,
  logUpdateV2: logUpdateV2,
  LazyStructWriter: LazyStructWriter,
  ...
  structSkipRefNumber: structSkipRefNumber,
  Skip: Skip
});
...
const compare = users => {
  ...
  for (let i = 0; i < users.length - 1; i++) {
    ...
    t__namespace.compare(userTextValues[i], userTextValues[i + 1]);
    t__namespace.compare(getStateVector(users[i].store), getStateVector(users[i + 1].store));
...
const testUndoManager = tc => {
  ...
  t__namespace.assert(posBeforeAnchor === 1 && posBeforeHead === 2);
  yUndoManager.redo();
  t__namespace.assert(
  ...
  binding.destroy();
  yUndoManager.redo();
  t__namespace.assert(ytext.toString() === 'ac');
  yUndoManager.undo();
  t__namespace.assert(ytext.toString() === 'abc');
};
...
const cmChanges = [
  ...
  (y, gen, cm) => { // insert text
    const insertPos = prng__namespace.int32(gen, 0, cm.getValue().length);
    const text = charCounter++ + prng__namespace.utf16String(gen, 6);
  ...
  (y, gen, cm) => { // delete text
    const insertPos = prng__namespace.int32(gen, 0, cm.getValue().length);
    const overwrite = prng__namespace.int32(gen, 0, cm.getValue().length - insertPos);
  ...
  (y, gen, cm) => { // replace text
    const insertPos = prng__namespace.int32(gen, 0, cm.getValue().length);
    const overwrite = math__namespace.min(prng__namespace.int32(gen, 0, cm.getValue().length - insertPos), 3);
